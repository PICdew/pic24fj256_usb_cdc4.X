...
/** V A R I A B L E S ********************************************************/
volatile unsigned char cdc_data_tx[CDC_MAX_COUNT][CDC_DATA_IN_EP_SIZE] IN_DATA_BUFFER_ADDRESS_TAG;
volatile unsigned char cdc_data_rx[CDC_MAX_COUNT][CDC_DATA_OUT_EP_SIZE] OUT_DATA_BUFFER_ADDRESS_TAG;
...
uint8_t cdc_trf_state[CDC_MAX_COUNT];         // States are defined cdc.h
POINTER pCDCSrc[CDC_MAX_COUNT];            // Dedicated source pointer
POINTER pCDCDst[CDC_MAX_COUNT];            // Dedicated destination pointer
uint16_t cdc_tx_len[CDC_MAX_COUNT];            // total tx length
uint8_t cdc_mem_type[CDC_MAX_COUNT];          // _ROM, _RAM

USB_HANDLE CDCDataOutHandle[CDC_MAX_COUNT];
USB_HANDLE CDCDataInHandle[CDC_MAX_COUNT];
...
void USBCheckCDCRequest(void)
{
     ...
     */
    id = 0;
    while(id < CDC_MAX_COUNT && 
          SetupPkt.bIntfID != CDC_COMM_INTF_ID(id) &&
          SetupPkt.bIntfID != CDC_DATA_INTF_ID(id)) {
        id++;
    }
    ...
    switch(SetupPkt.bRequest)
    {
        ...
        case SET_LINE_CODING:
            ...
            outPipes[0].pDst.bRam = (uint8_t*) UART_GetLineCodingById(id);
            outPipes[0].pFunc = LINE_CODING_PFUNC;
            outPipes[0].info.bits.busy = 1;
            outPipes[0].context = UART_GetObjById(id);
            break;
            
        case GET_LINE_CODING:
            USBEP0SendRAMPtr(
                (uint8_t*) UART_GetLineCodingById(id),
                LINE_CODING_LENGTH,
...
void CDCInitEP(void)
{
     ...
     *          sent.
     */
    for (i = 0; i < CDC_MAX_COUNT; i++) {
        USBEnableEndpoint(CDC_COMM_EP(i),USB_IN_ENABLED|USB_HANDSHAKE_ENABLED|USB_DISALLOW_SETUP);
        USBEnableEndpoint(CDC_DATA_EP(i),USB_IN_ENABLED|USB_OUT_ENABLED|USB_HANDSHAKE_ENABLED|USB_DISALLOW_SETUP);

        CDCDataOutHandle[i] = USBRxOnePacket(CDC_DATA_EP(i),(uint8_t*)&cdc_data_rx[i],sizeof(cdc_data_rx[i]));
        CDCDataInHandle[i] = NULL;
    

        #if defined(USB_CDC_SUPPORT_DSR_REPORTING)
            CDCNotificationInHandle = NULL;
            mInitDTSPin(i);  //Configure DTS as a digital input
...
bool USBCDCEventHandler(USB_EVENT event, void *pdata, uint16_t size)
{
    ...
    /* Search for corresponding CDC id */
    while (id < CDC_MAX_COUNT && 
           pdata != CDCDataOutHandle[id] && pdata != CDCDataInHandle[id]) {
        id++;
    }
    ...
    if (id < CDC_MAX_COUNT) {
        ...
        {  
            case EVENT_TRANSFER_TERMINATED:
                if(pdata == CDCDataOutHandle[id])
                {
                    CDCDataOutHandle[id] = 
                        USBRxOnePacket(CDC_DATA_EP(id), (uint8_t*)&cdc_data_rx[id], sizeof(cdc_data_rx[id]));
                }
                if(pdata == CDCDataInHandle[id])
                {
                    //flush all of the data in the CDC buffer
                    cdc_trf_state[id] = CDC_TX_READY;
                    cdc_tx_len[id] = 0;
                }
                break;
            default:
...
/**********************************************************************************
  ...
  **********************************************************************************/
uint16_t getsUSBUSART(uint8_t id, uint8_t *buffer, uint16_t len)
{
    cdc_rx_len = 0;
    
    if(!USBHandleBusy(CDCDataOutHandle[id]))
    {
        /*
         * Adjust the expected number of BYTEs to equal
         * the actual number of BYTEs received.
         */
        if(len > USBHandleGetLength(CDCDataOutHandle[id]))
            len = USBHandleGetLength(CDCDataOutHandle[id]);
        
        /*
         * Copy data from dual-ram buffer to user's buffer
         */
        for(cdc_rx_len = 0; cdc_rx_len < len; cdc_rx_len++)
            buffer[cdc_rx_len] = cdc_data_rx[id][cdc_rx_len];

        /*
         * Prepare dual-ram buffer for next OUT transaction
         */

        CDCDataOutHandle[id] = USBRxOnePacket(CDC_DATA_EP(id),(uint8_t*)&cdc_data_rx[id],sizeof(cdc_data_rx[id]));
 ...
 *****************************************************************************/
void putUSBUSART(uint8_t id, uint8_t *data, uint16_t  length)
{
    /*
    ...
    {
        mUSBUSARTTxRam(id, (uint8_t*)data, length);     // See cdc.h
    }
    USBUnmaskInterrupts();
}//end putUSBUSART
...
void putsUSBUSART(uint8_t id, char *data)
{
    uint8_t len;
    char *pData;
     ...
     */
    mUSBUSARTTxRam(id, (uint8_t*)data, len);     // See cdc.h
    USBUnmaskInterrupts();
}//end putsUSBUSART
...
/**************************************************************************
  ...
  **************************************************************************/
void putrsUSBUSART(uint8_t id, const const char *data)
{
    uint8_t len;
    ...
    mUSBUSARTTxRom(id, (const uint8_t*)data,len); // See cdc.h
    USBUnmaskInterrupts();
...
void CDCTxService(uint8_t id)
{
    ...
    if(USBHandleBusy(CDCDataInHandle[id])) 
    {
        USBUnmaskInterrupts();
        return;
    }
     ...
     */
    if(cdc_trf_state[id] == CDC_TX_COMPLETING)
        cdc_trf_state[id] = CDC_TX_READY;
     ...
     */
    if(cdc_trf_state[id] == CDC_TX_BUSY_ZLP)
    {
        CDCDataInHandle[id] = USBTxOnePacket(CDC_DATA_EP(id),NULL,0);
        //CDC_DATA_BD_IN.CNT = 0;
        cdc_trf_state[id] = CDC_TX_COMPLETING;
    }
    else if(cdc_trf_state[id] == CDC_TX_BUSY)
    {
        /*
         * First, have to figure out how many byte of data to send.
         */
    	if(cdc_tx_len[id] > sizeof(cdc_data_tx[id]))
    	    byte_to_send = sizeof(cdc_data_tx[id]);
    	else
    	    byte_to_send = cdc_tx_len[id];

        /*
         * Subtract the number of bytes just about to be sent from the total.
         */
    	cdc_tx_len[id] = cdc_tx_len[id] - byte_to_send;
    	  
        pCDCDst[id].bRam = (uint8_t*)&cdc_data_tx[id]; // Set destination pointer
        
        i = byte_to_send;
        if(cdc_mem_type[id] == USB_EP0_ROM)            // Determine type of memory source
        {
            while(i)
            {
                *pCDCDst[id].bRam = *pCDCSrc[id].bRom;
                pCDCDst[id].bRam++;
                pCDCSrc[id].bRom++;
                i--;
        ...
        else
        {
            while(i)
            {
                *pCDCDst[id].bRam = *pCDCSrc[id].bRam;
                pCDCDst[id].bRam++;
                pCDCSrc[id].bRam++;
                i--;
            }
        ...
        if(cdc_tx_len[id] == 0)
        {
            if(byte_to_send == CDC_DATA_IN_EP_SIZE)
                cdc_trf_state[id] = CDC_TX_BUSY_ZLP;
            else
                cdc_trf_state[id] = CDC_TX_COMPLETING;
        }//end if(cdc_tx_len...)
        CDCDataInHandle[id] = USBTxOnePacket(CDC_DATA_EP(id), (uint8_t*)&cdc_data_tx[id], byte_to_send);
